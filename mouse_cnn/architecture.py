...
data_folder = '/allen/programs/mindscope/workgroups/tiny-blue-dot/mousenet/Mouse_CNN/data/'
class Architecture(Data):
    ...
    def __init__(self, data_folder=data_folder,recurrent = False):
        super(Architecture, self).__init__(data_folder=data_folder)
        self.targets = _get_targets(self, data_folder=data_folder,recurrent = recurrent)
        self.channels = {}
    ...
    def get_kernel_peak_probability(self, source_area, source_layer, target_area, target_layer):
        if source_area == target_area: # from interlaminar hit rates
            print("source_area equals target area")    
            ...
            print('%s%s->%s%s: dw=%s, dp=%s eij=%s source_channels=%s,radius=%s'%(source_area,  source_layer,  target_area, target_layer, d_w, d_p,e_ij,source_channels,np.sum(np.exp(-radius**2/2/d_w**2))))
...
def _get_targets(data, data_folder=data_folder,recurrent = False):
    ...
    for area in data.get_areas():
        if data.get_hierarchical_level(area) > 0:
            for layer in data.get_layers():
                ...
                targets[_get_name(area, layer)] = Target(area, layer, external_in_degree=in_degree, data_folder=data_folder,recurrent=recurrent)
